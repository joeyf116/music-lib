...
export default function DesktopSidebar() {
  ...
  return (
    <aside
      ...
      style={{ backgroundColor: '#0d0d0d', borderRight: '1px solid var(--color-border)' }}
    >
      <div className="px-4 pt-5 pb-4" style={{ borderBottom: '1px solid var(--color-border)' }}>
        <div className="flex items-center gap-2.5">
          <div className="w-7 h-7 rounded flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'var(--color-accent)' }}>
      ...
      <nav className="flex-1 px-2 py-3 flex flex-col gap-0.5" aria-label="Main navigation">
        {NAV.map(({ to, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute left-0 inset-y-1 w-0.5 rounded-r" style={{ backgroundColor: 'var(--color-accent)' }} />
              )}
              <Icon size={15} />
              {label}
            </Link>
