...
import TabNotation from './TabNotation.jsx'
import FingeringDiagram from './FingeringDiagram.jsx'
...
function generateSVGFromDiagram(entry) {
  // Return a simple SVG placeholder with name text
...
export default function DetailView({ entry, entries, onClose, onSelect }) {
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h2 className="font-bold text-base leading-tight truncate" style={{ color: 'var(--color-text)' }}>
                  {entry.name}
                </h2>
                {entries && (
          ...
          <div className="flex flex-col gap-6 p-5">
              ...
              >
                {entry.formula && (
                  <p className="text-sm font-mono" style={{ color: 'var(--color-text)' }}>
                    <span className="font-semibold" style={{ color: 'var(--color-muted)' }}>Formula: </span>
                    <span style={{ color: 'var(--color-accent)' }}>{entry.formula}</span>
                  </p>
                )}
                {entry.context && (
                  <p className="text-sm mt-1" style={{ color: 'var(--color-muted)' }}>{entry.context}</p>
                )}
              </div>
            )}
            ...
            >
              ...
              <button
                onClick={() =>
                  downloadTxt(`${entry.id}.txt`, metaText)
                }
                className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-colors"
                style={{
                  backgroundColor: 'var(--color-bg)',
