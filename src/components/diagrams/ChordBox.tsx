...
export default function ChordBox({ diagram, leftHanded = false }: ChordBoxProps) {
  ...
  // For left-handed, mirror string index
    ...
    >
      ...
      {Array.from({ length: C_STRINGS }, (_, s) => (
        <line key={s}
          ...
          stroke="#8a8070"
          strokeWidth={[2.8, 2.2, 1.8, 1.4, 1.1, 0.8][leftHanded ? s : C_STRINGS - 1 - s]}
        />
      ))}
      ...
      {positions.map((pos: DiagramPosition, idx: number) => {
        ...
        if (pos.fret === -1) {
          return (
            <text key={idx} x={x} y={CT - 12} textAnchor="middle" fontSize="16" fill="#8a8070">×</text>
          )
        }
        if (pos.fret === 0) {
          return (
            <circle key={idx} cx={x} cy={CT - 13} r={6}
              fill="none"
